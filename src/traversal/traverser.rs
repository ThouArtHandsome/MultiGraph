...
///
/// * `labels` holds the `as(…)` labels active at the current step;
///   the traversal engine uses them to build label→position maps for
///   `select()` and `path()` evaluation.  `None` when no labels are attached.
///   Stack-allocates up to 2 labels without heap allocation.
///
/// * `parent` is a back-pointer to the traverser at the previous step, forming
///   a persistent tree (child → parent).  Following the chain and collecting
///   `(value, labels)` pairs reconstructs the full traversal history.
///   Allocated only when path tracking is active.
#[derive(Debug, Clone)]
